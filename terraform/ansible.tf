...
resource "local_file" "ansible_host" {
  ...
  content  = "[all:vars]\nansible_python_interpreter=/usr/bin/python3\nansible_user=${var.ssh_user}\nansible_ssh_common_args='-o StrictHostKeyChecking=no'\n\n[master]\nmaster_node ansible_host=${azurerm_linux_virtual_machine.myterraformvm[0].public_ip_address}\n\n[workers]\nworker1_node ansible_host=${azurerm_linux_virtual_machine.myterraformvm[1].public_ip_address}\nworker2_node ansible_host=${azurerm_linux_virtual_machine.myterraformvm[2].public_ip_address}\n\n[nfs]\nnfs_node ansible_host=${azurerm_linux_virtual_machine.myterraformvm[3].public_ip_address}"
...
resource "local_file" "public_ips" {
  ...
  filename = "../ansible/group_vars/public_ips.yaml"
}
